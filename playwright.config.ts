...
export default defineConfig({
  ...
  use: {
    ...
    /* The attribute to use for test IDs. */
    testIdAttribute: 'data-test', // tells getByTestId() to read data-test="..."
  },

  /* Configure projects for major browsers */
  projects: [
   {
    name: 'setup',                       // runs first, logs in once
    testMatch: /.*\.setup\.ts/,          // only picks up files ending in .setup.ts
    use: {
  ...
  },
  {
    name: 'api',
    testDir: './tests/api',
    use: { baseURL: 'https://api.practicesoftwaretesting.com' }, // no storageState, API tests handle their own token
  },
  {
    name: 'ui',
    testDir: './tests/ui',
